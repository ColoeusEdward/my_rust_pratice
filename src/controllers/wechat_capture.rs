...
static PADDLE_OCR_ENGINE: OnceLock<Mutex<Option<PaddleOcrEngine>>> = OnceLock::new();
...
fn run_umi_ocr(image_path: &Path) -> Result<String, String> {
    let state = PADDLE_OCR_ENGINE.get_or_init(|| Mutex::new(None));
    let mut guard = state.lock().unwrap();

    if guard.is_none() {
        *guard = Some(PaddleOcrEngine::start()?);
    }

    guard
        .as_mut()
        .ok_or_else(|| "PaddleOCR-json 引擎未初始化".to_string())?
